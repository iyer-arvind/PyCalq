...
def solve(eqns, vardict, pardict):
    ...
    vals = fmin(errsq, var_m,
                tuple(pardict.values()),
                ftol=1e-12, maxfun=1000000, maxiter=1000000)

    return dict((n, m if u.dimensionless else m*u)
                for n, m, u in zip(var_n, vals, var_u))
        

if __name__ == '__main__':
    from argparse import ArgumentParser
    parser = ArgumentParser('Simultaneous NonLinear Equation Solver with Units')
